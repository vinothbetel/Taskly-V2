...
interface TasksContextType {
  ...
  toggleSubtaskStatus: (taskId: string, subtaskId: string) => Promise<void>; // New
...
export function TasksContextProvider({ children }: { children: ReactNode }) {
  ...
  const sortTasks = (tasksArray: Task[]): Task[] => {
    return tasksArray.sort((a, b) => {
      // Primary sort: lastCommentedAt (most recent first)
      const aCommentTime = a.lastCommentedAt ? new Date(a.lastCommentedAt).getTime() : 0;
      const bCommentTime = b.lastCommentedAt ? new Date(b.lastCommentedAt).getTime() : 0;
      if (aCommentTime !== bCommentTime) return bCommentTime - aCommentTime;
      ...
      // Tertiary sort: createdAt (most recent first)
  ...
  const getTasksCompletedOnDate = (date: Date): Task[] => {
    ...
    return tasks.filter(task => {
      ...
      const taskDate = new Date(task.completedAt as string); // Cast to string
  ...
  const getLongestStreak = (): number => {
    if (tasks.length === 0) return 0;
    const completedDates = [...new Set(tasks.filter(t => t.status === 'completed' && t.completedAt).map(t => new Date(t.completedAt as string).setHours(0, 0, 0, 0)))].sort((a,b) => a - b); // Cast to string
